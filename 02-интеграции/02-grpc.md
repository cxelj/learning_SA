...
**Сценарий:** Два микросервиса обмениваются данными о пользователях 500 раз в секунду. Латентность критична (< 10 мс).

**Задание:**
1. Выберите между REST и gRPC. Обоснуйте выбор.
2. Опишите, что такое proto-файл и как выглядит определение сервиса для этого сценария.
3. Какие стриминговые паттерны (unary, server-streaming, client-streaming, bidirectional) подойдут для: (а) получения списка пользователей, (б) уведомлений о изменениях профиля в реальном времени?
4. Что произойдёт, если один из сервисов будет на Python, а другой на Go?
